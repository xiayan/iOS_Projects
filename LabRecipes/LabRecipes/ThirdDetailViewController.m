...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.editing && indexPath.section == INGREDIENTS_SECTION) {
        if (indexPath.row < [allIngredients count]) {
            cell.backgroundColor = (indexPath.row % 2 == 0) ? 
            [UIColor colorWithRed:0.892 green:0.893 blue:0.892 alpha:1.0] : [UIColor whiteColor];
        }
    } else {
...
//        int deletingIndex = [ingredient.displayOrder intValue];
        ...
        /*
        for (Ingredient *singleIngredient in allIngredients) {
            if ([singleIngredient.displayOrder intValue] > deletingIndex) {
                int newOrder = [singleIngredient.displayOrder intValue] - 1;
                singleIngredient.displayOrder = [NSNumber numberWithInt:newOrder];
            }
        }
        */
    }
